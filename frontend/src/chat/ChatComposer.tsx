...
export default function ChatComposer({ draft, onDraft, onSubmit, disabled, name }: {
  ...
  return (
    <form onSubmit={onSubmit} className="relative shrink-0 border-t border-white/10 bg-bg-0/90 p-3 sm:p-4 backdrop-blur-md z-20">
      ...
      <div className="mx-auto flex items-end gap-3 max-w-[860px] w-full">
        <textarea
          ref={textareaRef}
          ...
          rows={1}
          className="min-w-0 flex-1 min-h-[50px] max-h-48 resize-none overflow-y-auto border border-white/15 bg-black/80 px-4 py-3 text-[14.5px] leading-6 text-white placeholder:text-text-3 focus-visible:outline-2 focus-visible:outline-redline focus-visible:outline-offset-2 font-mono disabled:opacity-50"
        />
